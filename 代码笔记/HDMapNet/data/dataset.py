...
class HDMapNetDataset(Dataset):
    def __init__(self, version, dataroot, data_conf, is_train):
        ...
        # TODO: patch为配置区域的长宽； data_conf['xbound'][2]表示间隔，因此canvas表示网格数
        ...
        # scene是20s的视频数据
        self.scenes = self.get_scenes(version, is_train)  # -> Dict[str, List[str]]
        # sample是scene中某个时间戳代表的一帧图像
    ...
    def get_imgs(self, rec):
        ...
        for cam in CAMS:
            ...
            # resize 是缩放的比例，resize_dims 是缩放后的维度
    ...
    def get_vectors(self, rec):
        # location 实际上是MAPS的名称， = ['boston-seaport', 'singapore-hollandvillage','singapore-onenorth', 'singapore-queenstown']
        location = self.nusc.get(
            "log", self.nusc.get("scene", rec["scene_token"])["log_token"]
        )["location"]
...
class HDMapNetSemanticDataset(HDMapNetDataset):
    def __init__(self, version, dataroot, data_conf, is_train):
        super(HDMapNetSemanticDataset, self).__init__(
            version, dataroot, data_conf, is_train
        )
    ...
    def __getitem__(self, idx):
        ...
        lidar_data, lidar_mask = self.get_lidar(rec)
        car_trans, yaw_pitch_roll = self.get_ego_pose(rec)
        semantic_masks, instance_masks, _, _, direction_masks = self.get_semantic_map(
            rec
        )
        return (
            imgs,
...
def semantic_dataset(version, dataroot, data_conf, bsz, nworkers):
    # DataLoader:
    ## 复写__getitem__(self, idx): 通过索引index访问数据，能够同时返回tensor类型数据和对应的标签
    ## 复写__len__(self)：获取数据的个数
