...
5. 文章的缺陷：
   ...
   + 较难推理和部署：利用自回归解码器顺序预测点，导致较长的推理时间。同时，直接推理到矢量化需要大量数据，很难收敛；
...
2. 论文特点：
   ...
   + 网络的真值**匹配**：采用分层匹配方法，将地图元素集合视为固定大小，依次进行实例级匹配和Point-level匹配。
...
4. 一些没有理解的点：
   + MapTR对真值数据进行均匀采样（20个点），而非VectorMapNet 推崇的在曲率变化超过一定阈值的位置对顶点进行采样。不清楚为什么采用这个方案；
